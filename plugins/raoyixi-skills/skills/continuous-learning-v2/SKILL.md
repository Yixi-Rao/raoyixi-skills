...
## Codex Local Adaptation

This installation is available to Codex as a skill, but the upstream automatic observation hooks are Claude Code-specific. Do not copy the `~/.claude/settings.json` snippets into Codex. See `CODEX_ADAPTATION.md` in this skill directory for the local Codex workflow and manual CLI commands.
...
This packaged Codex plugin does not auto-install hooks. Treat `hooks/observe.sh` as a reusable script only.

If the user explicitly asks to wire observation hooks in Codex, use `$codex-hook` to add narrow entries to `$HOME/.codex/hooks.json` and validate the hook with representative stdin. Do not copy Claude Code `settings.json` snippets into Codex.
